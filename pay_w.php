...
<body>
    <div class="receipt">
        ...
        $id = $_SESSION["userid"];
        $pay_point = isset($_GET["point"]) ? intval($_GET["point"]) : 0;
        $pay_sum = isset($_GET["sum1"]) ? intval($_GET["sum1"]) : 0;
        $final_payment = $pay_sum - $pay_point;

        echo "<p>$final_payment 원 결제 완료 되었습니다.</p>";
        ...
        // 현재 포인트를 업데이트
        $new_point = $point - $pay_point + floor($pay_sum * 0.025);
